...
def _chunk_message(text: str, max_len: int = 3800):
    """Split into chunks safe for Telegram's 4096-char limit, breaking only
    on line boundaries so an HTML tag (like <a href=...>...</a>) is never
    cut in half across two messages."""
    lines = text.split("\n")
    chunks, current = [], []
    current_len = 0
    for line in lines:
        # +1 accounts for the newline that will join it back
